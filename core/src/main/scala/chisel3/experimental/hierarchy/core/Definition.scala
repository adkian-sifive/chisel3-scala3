...
object Definition {
  ...
  /** A construction method to build a Definition of a Module
    ...
    */
  def apply[T <: BaseModule & IsInstantiable](proto: => T): Definition[T] = do_apply(proto)
  ...
  def do_apply[T <: BaseModule & IsInstantiable](
    ...
    val dynamicContext = {
      ...
      new DynamicContext( // todo fix when updated
        Nil,
...
/** Stores a [[Definition]] that is imported so that its Instances can be
  * compiled separately.
  */
case class ImportDefinitionAnnotation[T <: BaseModule & IsInstantiable](
